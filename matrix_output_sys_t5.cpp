...
int main()
{
    ...
    for (int j = 0;; j++)
    {
        int key = inputselection(getkeyboard());
...
void blocks_collision_test(int matrix[12][22][4], int matrix_t[12][22][4], int checking_instruction)
{
    ...
    here:
        if (recode5block(matrix, recodedblock_2, kernel) == 0)
        {
            break;
        }
        ...
        if (newspace_check(matrix, recodedblock_1) == 1)
        {
            ...
            putback(matrix, recodedblock_2, kernel);
...
int block_rotation(int matrix[12][22][4], int matrix_t[12][22][4], int mode, int bloclktype)
{
    ...
    int a[2] = {0}; // in this function kernel is a
    int nine_1[9][4] = {0};

    if (bloclktype != 4 && bloclktype != 3)
    {
        int recodedblock[4][6];
        search_r(matrix, a);
        if (a[1] == 1)
        {
            recode5block(matrix, recodedblock, a);
            if (newspace_check(matrix, recodedblock) == 0)
            {
                moving_down_test(matrix);
